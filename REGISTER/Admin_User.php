  ...
  <div class="col-sm-1"></div>
   ...
   <table class="table table-bordered" >
    <tr style="background-color:green;color:white">
      <td>Login ID</td><td>User Name</td><td>Phone</td><td>Email</td><td>Status</td><td>Action
      </td>
    </tr>
<?php
...
$result=mysqli_query($db,"select * from user_detail");
while($row=mysqli_fetch_array($result))
{
  echo "<tr>";
  echo "<td>$row[0]</td><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td>";
  if($row[4]=="Active")
   echo "<td><a href='Admin_Block.php?id=$row[0]&status=$row[4]'>Block</a></td>";
  else
   echo "<td><a href='Admin_Block.php?id=$row[0]&status=$row[4]'>Active</a></td>";
  echo "</tr>";
}
mysqli_close($db);
?>
